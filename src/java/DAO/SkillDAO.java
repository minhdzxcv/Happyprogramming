...
public class SkillDAO {
    ...
    public static boolean createSkill(String name, boolean active, String avatar, String description) throws Exception {
        ...
        PreparedStatement ps = dbo.prepareStatement("INSERT INTO Skills (SkillName, enable, [image], [Description]) VALUES (?, ?, ?, ?)");
        ps.setString(1, name);
    ...
    public static boolean UpdateSkill(int id, String name, boolean active, String avatar, String description) throws Exception {
        Connection dbo = DatabaseUtil.getConn();
        PreparedStatement ps = dbo.prepareStatement("UPDATE Skills SET SkillName = ?, enable = ?, [Description] = ?" + (avatar != null ? ", [image] = ?" : "") +" WHERE SkillID = ?");
        ps.setString(1, name);
        ps.setInt(2, active ? 1 : 0);
