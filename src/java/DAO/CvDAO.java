...
public class CvDAO {
    ...
    public static boolean updateCV(int CvID, String ProfessionIntro, String Description, int MoneyOfSlot) throws Exception {
        ...
        try {
            ...
            ps.setInt(5, CvID);
            ps.executeUpdate();
            PreparedStatement ps2 = dbo.prepareStatement("UPDATE [Mentor] SET [status] = 'Draft' WHERE [CvID] = ?");
            ps2.setInt(1, CvID);
            ps2.executeUpdate();
            dbo.commit();
            dbo.close();
    ...
    public static boolean updatePriceOfSlot(int CvID, int MoneyOfSlot) throws Exception {
        Connection dbo = DatabaseUtil.getConn();
        try {
            PreparedStatement ps = dbo.prepareStatement("UPDATE [CV] SET [MoneyOfSlot] = ? WHERE [CvID] = ?");
            ps.setInt(1, MoneyOfSlot);
            ps.setInt(2, CvID);
            ps.executeUpdate();
            dbo.commit();
            dbo.close();
