...
public class ScheduleDAO {
    ...
    public static void updateSlotNew(int id, String link, float hour, Timestamp start, int scheId) throws Exception {
        Connection dbo = DatabaseUtil.getConn();
        try {
            PreparedStatement ps = dbo.prepareStatement("UPDATE [Slots] SET [Link] = ?, [Time] = ?, [startAt] = ?, scheduleID=?, status='Pending' WHERE [SlotID] = ?");
            ps.setString(1, link);
            ps.setFloat(2, hour);
            ps.setTimestamp(3, start);
            ps.setInt(4, scheId);
...
//    }
    ...
    public static Schedule getScheduleByIdAndSlot(int mentorID, int scheduleId) {
        ArrayList<Schedule> schedules = new ArrayList<>();
        Connection dbo = DatabaseUtil.getConn();
        PreparedStatement ps = null;
        ResultSet rs = null;
        try {
            String query = "select Sch.* from Slots as S \n"
    ...
    public static int createSchedule(Schedule schedule) {
        ...
        try {
            ...
            ps = dbo.prepareStatement(query, Statement.RETURN_GENERATED_KEYS);
