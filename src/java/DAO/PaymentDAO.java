...
public class PaymentDAO {
    ...
    public static void confirmPayment(int id) throws Exception {
        ...
        PreparedStatement ps = dbo.prepareStatement("UPDATE [User] SET [wallet] = [wallet] + (SELECT [price] FROM [Payment] WHERE [ID] = ?) WHERE [UserID] = (SELECT [ReceiverID] FROM [Payment] WHERE [ID] = ?)");
        ps.setInt(1, id);
        ps.setInt(2, id);
        ps.executeUpdate();
